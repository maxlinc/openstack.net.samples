...
namespace Samples.openstack.net
{
    public static class openstack
    {
        ...
        public static void RunAll(string username, string apikey)
        {
            ...
            SpinUpServerInNetwork("server_name", "6d18a62a-0180-4cee-b638-71eedb83d8d5", "performance1-2", "IAD", "00000000-0000-0000-0000-000000000000", cloudId);
        ...
        public static CloudIdentity CreateIdentity(string username, string apikey)
        {
            try
            {
                ...
                CloudIdentityProvider bar = new CloudIdentityProvider(foo);
        ...
        private static void ImagesToConsole(System.Collections.IEnumerable images, string region)
        {
            foreach (ServerImage image in images)
            {
                Console.WriteLine("Following image is in Region {0}", region);
        ...
        public static void ListObjects(CloudIdentity cloudId)
        {
            if (PromptUser("Would you like to list the objects in a Container?")) {
                Console.WriteLine("Name of Container: ");
                string containerName = Console.ReadLine();
                CloudFilesProvider cfp = new CloudFilesProvider(cloudId);
                IEnumerable<ContainerObject> objectList = cfp.ListObjects(containerName);
                foreach (ContainerObject cobj in objectList)
                {
                    Console.WriteLine("Object name: {0}, type: {1}", cobj.Name, cobj.ContentType);
                }
            }
        }
        ...
        public static void ListFlavors(CloudIdentity cloudId)
        {
            try
            {
                if (PromptUser("Do you want to list all the flavors?"))
                {
                    ...
                    CloudServersProvider csp = new CloudServersProvider(cloudId);
                    System.Collections.IEnumerable flavors = csp.ListFlavorsWithDetails(region: "IAD");
                    Console.Write("Done.\n");
        ...
        public static void SpinUpServerInNetwork( string servername, string imageId, string flavorId, string region, string networkId, CloudIdentity cloudId)
        {
            try
            {
                ...
                Personality p = new Personality(@"C:\cloud-automation\bootstrap.cmd", "powershell.exe Set-ExecutionPolicy Unrestricted & ren C:\\mystuff\\enablePS_1.txt enablePS_1.ps1 & ren C:\\mystuff\\enablePS_2.txt enablePS_2.ps1 & powershell.exe C:\\mystuff\\enablePS_1.ps1 & powershell.exe C:\\mystuff\\enablePS2.ps1 & netsh advfirewall firewall set rule group=\"remote administration\" new enable=yes & netsh advfirewall firewall add rule name=\"WinRM Port\" dir=in action=allow protocol=TCP localport=5985");
                Personality p2 = new Personality(@"C:\mystuff\enablePS_1.txt", "Set-Item WSMan:\\localhost\\Client\\TrustedHosts -Value \"*\"-Force");
                Personality p3 = new Personality(@"C:\mystuff\enablePS_2.txt", "New-ItemProperty -name LocalAccountTokenFilterPolicy -path HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System -PropertyType DWord -value 1");
                //Personality p2 = new Personality(@"C:\mystuff\enablePowershell.cmd", "Set-Item");

                DiskConfiguration dc = DiskConfiguration.Manual;
                List<string> nets = new List<string>();
                nets.Add( "00000000-0000-0000-0000-000000000000");
                nets.Add("60426d01-3cbb-44c1-8238-e68706bccaaf");
                NewServer newServer = csp.CreateServer(servername, imageId, networks: nets, flavor: flavorId, region: region, personality: new Personality[]{p,p2,p3}, diskConfig: dc);
